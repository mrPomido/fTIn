...
int main(int argc, char** argv){
    ...
    if(WSAStartup(MAKEWORD(2, 2), &d)){
        fprintf(stderr, "Failed to initialize socket(WINDOWS)\n");
        return 1;
    }
#endif

    if (argc < 3){
        fprintf(stderr, "usage: AppName hostname port");
        return 1;
    }
    ...
    if(!ISVALIDSOCKET(socket_peer)){
        fprintf(stderr, "socket() is failed: %d\n", GETSOCKETERRNO());
        return 1;
    }
    ...
    printf("Connected.\n");
    printf("To senddata, enter text, followed by enter.\n");
